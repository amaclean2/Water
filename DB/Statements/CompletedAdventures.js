const selectActivitiesByAdventureStatement =
  'SELECT CONCAT(u.first_name, " ", u.last_name) AS display_name, u.first_name, u.email, u.profile_picture_url, u.id AS user_id FROM completed_adventures AS ac INNER JOIN users AS u ON ac.creator_id = u.id INNER JOIN adventures AS a ON ac.adventure_id = a.id WHERE ac.adventure_id = ?'
const getCompletedAdventureByUserStatement =
  'SELECT a.difficulty, a.rating, a.coordinates_lat, a.coordinates_lng, a.adventure_name, a.adventure_type, a.nearest_city, a.id AS adventure_id FROM completed_adventures AS ac INNER JOIN users AS u ON ac.creator_id = u.id INNER JOIN adventures AS a ON ac.adventure_id = a.id WHERE ac.creator_id = ?'
